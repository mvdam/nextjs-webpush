...
Add info on Next generation settings?
Screenshots in EN?
Is layout file generated by default?
...
We import all the required types from `next/server`, the `web-push` API's, our `CONFIG` with the private and public keys in it and our newly created `in-memory-db`.
...
Example project Github: ... - has some extra debug features
